...
const linksReducer = (state = initialState, action) => {
  ...
  if (action.type === UPDATE_FETCHED) {
    ...
    addNextAction(extractContents(null, null));
    return newState;
  }
  ...
  if (action.type === MOVE_LINKS_ADD_STEP_COMMIT) {
    ...
    const newState = { ...state };
    for (const [listName, lnIds] of Object.entries<any>(successIdsPerLn)) {
      newState[listName] = _.update(newState[listName], ID, lnIds, STATUS, ADDED);
    ...
    if (toUnpinIds.length > 0) {
      addNextAction(unpinLinks(toUnpinIds));
      return newState;
    }
  ...
  if (action.type === EXTRACT_CONTENTS_COMMIT) {
    addNextAction(tryUpdateExtractedContents(action.payload));
    return state;
  }
  ...
  if (action.type === UPDATE_EXTRACTED_CONTENTS) {
    ...
    for (const [listName, lnLinks] of Object.entries<any>(successLinksPerLn)) {
  ...
  if (action.type === UPDATE_CUSTOM_DATA_COMMIT) {
    ...
    addNextAction(updateCustomDataDeleteStep(
      listName, fromLink, toLink, serverUnusedFPaths, localUnusedFPaths
    ));
    return newState;
  }
...
const _getLastLinks = (links) => {
  ...
  const lnksPerMid = {};
  for (const listName in links) {
    for (const link of Object.values<any>(links[listName])) {
      const { id } = link;
