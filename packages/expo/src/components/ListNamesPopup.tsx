...
const ListNamesPopup = () => {
  ...
  if (isAnimTypeB) {
    ...
    if (derivedMode === MODE_ADD_LINK) {
      const minHgt = 320; // hedHgt + (itmHgt*5) + 4
      popupHeight = Math.min(minHgt, hedHgt + (itmHgt * maxChildrenSize) + 4);
    } else {
    ...
    return (
      <React.Fragment>
        <View style={tailwind(`flex-row items-center justify-start ${viewClassNames}`)}>
          ...
          {[MODE_MOVE_LINKS, MODE_ADD_LINK].includes(derivedMode) && <TouchableOpacity onPress={onNewBtnClick} style={tailwind('h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center')}>
            <Svg style={tailwind('font-normal text-gray-500 blk:text-gray-300')} width={20} height={20} viewBox="0 0 20 20" fill="currentColor">
