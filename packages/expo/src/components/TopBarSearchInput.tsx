  ...
  render() {
    ...
    return (
      <View style={tailwind('ml-4 w-48')}>
        <TextInput onChange={this.onSearchInputChange} style={tailwind(`ml-0.5 w-full rounded-full border border-transparent bg-gray-100 pl-9 pr-6 text-sm font-normal text-gray-700 blk:bg-gray-800 blk:text-gray-200 ${inputClassNames}`)} placeholder="Search" placeholderTextColor={themeMode === BLK_MODE ? 'rgb(156, 163, 175)' : 'rgb(107, 114, 128)'} value={searchString} autoCapitalize="none" />
        ...
        <View style={tailwind('absolute inset-y-0 left-0 justify-center pl-3')}>
          <Svg style={tailwind('font-normal text-gray-400 blk:text-gray-400')} width={20} height={20} viewBox="0 0 24 24" fill="currentColor">
