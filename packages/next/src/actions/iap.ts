...
import {
  isFldStr, sleep, sample, getLatestPurchase, getValidPurchase, getResErrMsg,
} from '../utils';
...
export const requestPurchase = (product) => async (dispatch, getState) => {
  ...
  try {
    await iapApi.requestPurchase(product.id);
  } catch (error) {
...
const checkRequestPurchase = async (dispatch, getState) => {
  ...
  try {
    const statusResult = await getIapStatus(false);
