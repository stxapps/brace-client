...
import {
  ...
  updateSettingsPopup, updateSettingsViewId, updateLinkEditor,
} from '../actions/chunk';
import {
  LIST_NAMES_POPUP, TRASH, LIST_NAMES_MODE_CHANGE_LIST_NAME,
  LIST_NAMES_MODE_CHANGE_TAG_NAME, LIST_NAMES_MODE_MOVE_LINKS,
  LIST_NAMES_MODE_MOVE_LIST_NAME, LIST_NAMES_MODE_ADD_LINK, LIST_NAMES_ANIM_TYPE_POPUP,
...
const MODE_ADD_LINK = LIST_NAMES_MODE_ADD_LINK;
...
const ListNamesPopup = () => {
  ...
  const onLnItemBtnClick = (selectedListName) => {
    ...
    } else if (derivedMode === MODE_ADD_LINK) {
      dispatch(updateLinkEditor({ listName: selectedListName }));
    } else {
  ...
  if (isAnimTypeB) {
    ...
    if (derivedMode === MODE_ADD_LINK) {
      const minHgt = toPx('20rem'); // hedHgt + (itmHgt*5) + 0.25rem;
      popupHeight = Math.min(
        minHgt, hedHgt + (itmHgt * maxChildrenSize) + toPx('0.25rem')
      );
    } else {
    ...
    if ([
      MODE_CHANGE_LIST_NAME, MODE_CHANGE_TAG_NAME, MODE_ADD_LINK,
    ].includes(derivedMode)) {
      ...
      <div className={tailwind(`${viewClassNames}`)}>
        {children.map(obj => {
          ...
          if (derivedMode === MODE_MOVE_LIST_NAME) {
            ...
            disabled = [TRASH].includes(obj.listName);
            forwardDisabled = [TRASH].includes(obj.listName);
          }
          ...
          return (
            <div key={obj.listName} className={tailwind('flex w-full flex-row items-center justify-start')}>
              <button onClick={() => onLnItemBtnClick(obj.listName)} className={tailwind(`group flex min-w-0 flex-shrink flex-grow flex-row items-center pl-4 hover:bg-gray-100 focus:bg-gray-100 focus:outline-none blk:hover:bg-gray-700 blk:focus:bg-gray-700 ${btnClassNames}`)} disabled={disabled}>
                ...
                {(derivedMode === MODE_CHANGE_LIST_NAME && obj.listName in derivedUpdates) && <div className={tailwind('ml-1 mb-2.5 h-1.5 w-1.5 flex-shrink-0 flex-grow-0 rounded-full bg-blue-400')}></div>}
              </button>
  ...
  const _render = () => {
    ...
    if ([MODE_CHANGE_LIST_NAME, MODE_ADD_LINK].includes(derivedMode)) {
      rootName = 'Lists';
    } else if (derivedMode === MODE_CHANGE_TAG_NAME) {
      rootName = 'Tags';
    }
    ...
    return (
      <div className={tailwind('flex h-full w-full flex-col')}>
        <div className={tailwind(`flex w-full flex-shrink-0 flex-grow-0 items-center justify-start ${viewClassNames}`)}>
          ...
          {[MODE_MOVE_LINKS, MODE_ADD_LINK].includes(derivedMode) && <button onClick={onNewBtnClick} className={tailwind('group flex h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center focus:outline-none')}>
            <svg className={tailwind('h-5 w-5 rounded text-gray-500 group-hover:text-gray-700 group-focus:ring blk:text-gray-300 blk:group-hover:text-gray-100')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  if (isAnimTypeB) {
    ...
    panel = (
      <motion.div key="LNP_popup" style={popupStyle} className={tailwind('fixed inset-x-0 bottom-0 z-41 rounded-t-lg bg-white shadow-xl ring-1 ring-black/5 blk:bg-gray-800 blk:ring-white/25')} variants={bModalFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
        {_render()}
      </motion.div>
    );
  } else {
    ...
    panel = (
      <motion.div key="LNP_popup" style={popupStyle} className={tailwind('fixed z-41 overflow-auto rounded-lg bg-white shadow-xl ring-1 ring-black/5 blk:bg-gray-800 blk:ring-white/25')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
        {_render()}
      </motion.div>
    );
  }
  ...
  return (
    <AnimatePresence key="AP_lnPopup">
      <motion.button key="LNP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className={tailwind('fixed inset-0 z-40 h-full w-full cursor-default bg-black/25 focus:outline-none')} variants={bgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
