...
const PaywallPopup = () => {
  ...
  return (
    <View style={[tailwind('absolute inset-0 items-center justify-end pt-4 px-4 pb-20 shadow-xl z-40 sm:justify-center sm:p-0', safeAreaWidth), canvasStyle]}>
      ...
      <Animated.View style={[tailwind('w-full max-w-lg bg-white rounded-lg px-4 pt-5 pb-4 shadow-xl sm:my-8 sm:p-6', safeAreaWidth), popupStyle]}>
        <View style={tailwind('justify-start items-center')}>
          <View style={tailwind('items-center justify-center h-12 w-12 rounded-full bg-gray-100', safeAreaWidth)}>
            <Svg width={24} height={24} style={tailwind('text-gray-600 font-normal')} fill="currentColor" viewBox="0 0 20 20">
          ...
          <View style={tailwind('mt-3 sm:mt-5', safeAreaWidth)}>
            <Text style={tailwind('text-lg leading-6 font-medium text-gray-900 text-center')}>Purchase a subscription</Text>
            <View style={tailwind('mt-2')}>
              <Text style={tailwind('text-sm text-gray-500 font-normal text-center')}>Pin to the top is an extra feature. Please purchase a subscription to support us and unlock extra feature(s).</Text>
            </View>
          </View>
        </View>
        <View style={tailwind('mt-5 sm:mt-6 sm:flex-row sm:justify-center sm:items-center', safeAreaWidth)}>
          <TouchableOpacity onPress={onOkBtnClick} style={tailwind('w-full rounded-full border border-gray-800 py-2 bg-gray-800 sm:w-40', safeAreaWidth)}>
            <Text style={tailwind('text-base font-medium text-white text-center sm:text-sm', safeAreaWidth)}>More info</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={onCancelBtnClick} style={[tailwind('mt-3 w-full rounded-full border border-gray-400 bg-white sm:mt-0 sm:ml-14 sm:w-40', safeAreaWidth), cancelBtnStyle]}>
