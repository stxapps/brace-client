...
const SettingsPopupMisc = (props) => {
  ...
  const whtRBtnClassNames = themeMode === WHT_MODE ? 'border-blue-600' : 'border-gray-200';
  const whtRBtnInnerClassNames = themeMode === WHT_MODE ? 'bg-blue-600' : 'bg-gray-200';
  ...
  const blkRBtnClassNames = themeMode === BLK_MODE ? 'border-blue-600' : 'border-gray-200';
  const blkRBtnInnerClassNames = themeMode === BLK_MODE ? 'bg-blue-600' : 'bg-gray-200';
  ...
  const systemRBtnClassNames = themeMode === SYSTEM_MODE ? 'border-blue-600' : 'border-gray-200';
  const systemRBtnInnerClassNames = themeMode === SYSTEM_MODE ? 'bg-blue-600' : 'bg-gray-200';
  ...
  const customRBtnClassNames = themeMode === CUSTOM_MODE ? 'border-blue-600' : 'border-gray-200';
  const customRBtnInnerClassNames = themeMode === CUSTOM_MODE ? 'bg-blue-600' : 'bg-gray-200';
  ...
  return (
    <View style={tailwind('relative p-4 md:p-6')}>
      ...
      <View style={tailwind('mt-6 md:mt-0')}>
        <Text style={tailwind('text-base font-medium leading-4 text-gray-800')}>Appearance</Text>
        {systemText}
                  ...
                  <View style={tailwind('mt-1.5 sm:flex-row sm:items-center sm:justify-start')}>
                    <View style={tailwind('flex-row items-center justify-start')}>
                      ...
                      <TouchableOpacity ref={whtTimeBtn} onPress={onWhtTimeBtnClick} style={tailwind('ml-1 rounded-md border border-gray-300 bg-white px-3 py-1.5')} disabled={themeMode !== CUSTOM_MODE}>
                        <Text style={tailwind(`text-base font-normal leading-5 ${customInputClassNames}`)}>{whtTime}</Text>
                      </TouchableOpacity>
                    </View>
                    <View style={tailwind('mt-2 flex-row items-center justify-start sm:ml-4 sm:mt-0')}>
                      ...
                      <TouchableOpacity ref={blkTimeBtn} onPress={onBlkTimeBtnClick} style={tailwind('ml-1 rounded-md border border-gray-300 bg-white px-3 py-1.5')} disabled={themeMode !== CUSTOM_MODE}>
                        <Text style={tailwind(`text-base font-normal leading-5 ${customInputClassNames}`)}>{blkTime}</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                </View>
