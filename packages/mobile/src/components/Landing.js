...
class Landing extends React.PureComponent {
  ...
  renderSwiper(swiperHeight) {
    ...
    if (!styles.trulyText) styles.trulyText = [tailwind('mt-3 text-xl text-gray-700 font-normal text-center'), { lineHeight: 29 }];

    if (!styles.blockstackGrayBox) {
      const borderRadius = {
      ...
      styles.blockstackGrayBox = [tailwind('p-4 bg-gray-100 max-w-sm'), borderRadius];
    }
    ...
    if (!styles.quote) {
      ...
      styles.quote = [tailwind('absolute top-0 left-0 text-base text-indigo-200 font-normal opacity-50'), quoteTranslate];
    }
    ...
    return (
      <Swiper height={swiperHeight} showsPagination={true} showsButtons={false} activeDotStyle={tailwind('bg-gray-900')} paginationStyle={{ bottom: 10 }} loop={false}>
        ...
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          ...
          <Text style={tailwind('mt-3 text-lg text-gray-700 font-normal text-center leading-7')}>A lot of interesting, useful, and important stuff is {safeAreaWidth >= SM_WIDTH && safeAreaWidth < MD_WIDTH ? '\n' : ''}out there on the internet. {safeAreaWidth >= MD_WIDTH ? '\n' : ''}Brace.to helps you save them so that you will never miss anything.</Text>
        </View>
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          <View style={tailwind('flex-row max-w-md')}>
            <View style={tailwind('flex-shrink-0 flex-grow-0')}>
              <View style={tailwind('flex items-center justify-center h-12 w-12 bg-gray-700 rounded-md')}>
                <Svg style={tailwind('text-base text-white font-normal')} width={18} height={21} viewBox="0 0 47 56" fill="currentColor">
            ...
            <View style={tailwind('ml-4 flex-shrink flex-grow')}>
              ...
              <Text style={tailwind('mt-1.5 text-lg text-gray-700 font-normal leading-7')}>You found a long, useful, and important article you can't read it right now? Just save to Brace.to to read it later.</Text>
            </View>
          </View>
          <View style={tailwind('mt-8 flex-row max-w-md md:mt-12', safeAreaWidth)}>
            <View style={tailwind('flex-shrink-0 flex-grow-0')}>
              <View style={tailwind('flex items-center justify-center h-12 w-12 bg-blue-500 rounded-md')}>
                <Svg style={tailwind('text-base text-white font-normal')} width={20} height={20} viewBox="0 0 56 56" fill="currentColor">
            ...
            <View style={tailwind('ml-4 flex-shrink flex-grow')}>
              ...
              <Text style={tailwind('mt-1.5 text-lg text-gray-700 font-normal leading-7')}>You found a ridiculously cool and interesting website you want to check it out later? Just save to Brace.to to visit it later.</Text>
            </View>
          </View>
        </View>
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          <View style={tailwind('flex-row max-w-md')}>
            <View style={tailwind('flex-shrink-0 flex-grow-0')}>
              <View style={tailwind('flex items-center justify-center h-12 w-12 bg-orange-500 rounded-md')}>
                <Svg style={tailwind('text-base text-white font-normal')} width={28} height={21} viewBox="0 0 75 56" fill="currentColor">
            ...
            <View style={tailwind('ml-4 flex-shrink flex-grow')}>
              ...
              <Text style={tailwind('mt-1.5 text-lg text-gray-700 font-normal leading-7')}>You found an item on online shop you don’t want it now but might want to buy it later? Just save to Brace.to.</Text>
            </View>
          </View>
          <View style={tailwind('mt-8 flex-row max-w-md md:mt-12', safeAreaWidth)}>
            <View style={tailwind('flex-shrink-0 flex-grow-0')}>
            ...
            <View style={tailwind('ml-4 flex-shrink flex-grow')}>
              ...
              <Text style={tailwind('mt-1.5 text-lg text-gray-700 font-normal leading-7')}>Your friend’s just shared a video with you but you want to watch it tonight? Just save to Brace.to to watch it later.</Text>
            </View>
          </View>
        </View>
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          ...
          <Text style={styles.trulyText}>Your identity is truly yours. {safeAreaWidth >= SM_WIDTH ? '' : '\n'}Your data is truly yours.</Text>
        </View>
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          <View style={styles.blockstackGrayBox}>
        ...
        <View style={tailwind('px-4 justify-center items-center w-full h-full md:px-6 lg:px-8', safeAreaWidth)}>
          <View style={styles.blockstackGrayBox}>
            <Svg style={tailwind('text-base text-purple-blockstack font-normal')} width={51} height={56} viewBox="0 0 72 79" fill="currentColor">
        ...
        <View style={tailwind('px-8 justify-center items-center w-full h-full')}>
          <View style={tailwind('mt-6 max-w-sm')}>
            <Svg style={styles.quote} width={144} height={144} stroke="currentColor" fill="none" viewBox="0 0 144 144">
            ...
            <Text style={tailwind('text-xl text-gray-700 font-normal leading-8')}>Bring back control of your identity and your data one link at a time with Brace.to, powered by web 3.0 technology from Blockstack to make sure that your privacy cannot be compromised.</Text>
          </View>
        </View>
      </Swiper >
    );
    ...
    return (
      <TouchableOpacity onPress={() => this.props.signUp()} style={tailwind('py-5 items-center')}>
        <View style={tailwind('px-6 flex-row justify-center items-center h-12 bg-gray-900 rounded-full shadow-lg')}>
          ...
          <Svg style={tailwind('ml-2 text-base text-white font-normal')} width={6} height={13} viewBox="0 0 6 10" fill="currentColor">
