  ...
  return (
    <View style={tailwind('p-4 md:p-6')}>
      <View style={tailwind('border-b border-gray-200 md:hidden')}>
        ...
        <Text style={tailwind('pb-2 text-xl font-medium leading-5 text-gray-800')}>Lists</Text>
      </View>
      <View style={tailwind('hidden md:flex')}>
        <Text style={tailwind('text-base font-medium leading-4 text-gray-800')}>Lists</Text>
      </View>
    ...
    <React.Fragment>
      <View style={[tailwind('mt-1 flex-row items-center justify-start'), viewStyle]}>
        {(state.mode === MODE_VIEW && listNameObj === null) && <TouchableOpacity onPress={onAddBtnClick} style={tailwind('h-10 w-8 flex-shrink-0 flex-grow-0 flex-row items-center justify-start')}>
          <Svg style={tailwind('font-normal text-gray-500')} width={14} height={14} viewBox="0 0 14 14" fill="currentColor">
        ...
        {state.mode === MODE_EDIT && <TouchableOpacity onPressIn={onCancelBtnPress} onPress={onCancelBtnClick} style={tailwind('h-10 w-8 flex-shrink-0 flex-grow-0 flex-row items-center justify-start')}>
          <Svg style={tailwind('font-normal text-gray-500')} width={12} height={12} viewBox="0 0 12 12" fill="currentColor">
        ...
        <View style={tailwind('flex-shrink flex-grow')}>
          <TextInput ref={input} onFocus={onInputFocus} onBlur={onInputBlur} onChange={onInputChange} onSubmitEditing={onInputKeyPress} style={tailwind('w-full border-0 bg-white px-0 py-1 text-base font-normal leading-5 text-gray-600')} placeholder="Create new list" value={state.value} editable={!isBusy} />
          <Text style={[tailwind('absolute left-0 right-0 text-sm font-medium leading-5 text-red-600'), { bottom: Platform.OS === 'ios' ? -12 : -8 }]} numberOfLines={1} ellipsizeMode="tail">{state.msg}</Text>
        </View>
        {state.mode === MODE_EDIT && <TouchableOpacity onPressIn={onOkBtnPress} onPress={onOkBtnClick} style={tailwind('h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center')}>
          <Svg style={tailwind('font-normal text-gray-500')} width={16} height={12} viewBox="0 0 14 10" fill="currentColor">
        ...
        {(state.mode === MODE_VIEW && listNameObj !== null) && <TouchableOpacity onPress={onEditBtnClick} style={tailwind('hidden h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center sm:flex')} disabled={isBusy}>
          <Svg style={tailwind('font-normal text-gray-500')} width={16} height={16} viewBox="0 0 14 14" fill="currentColor">
        ...
        {(state.mode === MODE_VIEW && listNameObj !== null) && <TouchableOpacity onPress={onMoveUpBtnClick} style={tailwind('hidden h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center lg:flex')} disabled={isBusy}>
          <Svg style={tailwind('font-normal text-gray-500')} width={16} height={18} viewBox="0 0 14 16" fill="currentColor">
        ...
        {(state.mode === MODE_VIEW && listNameObj !== null) && <TouchableOpacity ref={menuBtn} onPress={onMenuBtnClick} style={tailwind('h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center')} disabled={isBusy}>
          <Svg style={tailwind('font-normal text-gray-500')} width={18} height={18} viewBox="0 0 20 20" fill="currentColor">
