...
const ListNamesPopup = () => {
  ...
  const _render = () => {
    ...
    return (
      <React.Fragment>
        ...
        {[MODE_MOVE_LINKS, MODE_MOVE_LIST_NAME].includes(mode) && <View style={tailwind(`flex-row items-center justify-end border-t border-gray-200 px-3 ${moveHereClassNames}`)}>
          <TouchableOpacity onPress={onMoveHereBtnClick} style={tailwind(`rounded-full border bg-white px-3 py-1.5 ${moveHereDisabled ? 'border-gray-300' : 'border-gray-400'}`)} disabled={moveHereDisabled}>
  ...
  if (animType === ANIM_TYPE_BMODAL) {
    ...
    panel = (
      <Animated.View style={[tailwind('absolute inset-x-0 bottom-0 rounded-t-lg border border-gray-100 bg-white shadow-xl'), popupStyle]}>
        {_render()}
      </Animated.View>
    );
  } else {
    ...
    panel = (
      <Animated.View style={[tailwind('absolute rounded-lg border border-gray-100 bg-white shadow-xl'), popupStyle]}>
        {_render()}
      </Animated.View>
    );
  }
