...
import {
  View, Text, TouchableOpacity, TextInput, Linking, Animated, Platform,
} from 'react-native';
import {
...
import {
  signIn, signOut, updatePopup, updateSearchString, updateBulkEdit,
} from '../actions';
import {
  DOMAIN_NAME,
  PROFILE_POPUP, SETTINGS_POPUP,
  ...
  _renderCommands() {
    ...
    return (
      <View style={tailwind('flex-row justify-end items-center')}>
        ...
        <Menu renderer={renderers.Popover} rendererProps={cache('TB_profileCommandMenuRendererProps', { preferredPlacement: 'bottom', anchorStyle: tailwind(anchorClasses) })} onOpen={this.onProfileBtnClick} onClose={this.onProfileCancelBtnClick}>
          <MenuTrigger>
          ...
          <MenuOptions customStyles={cache('TB_profileCommandMenuOptions', { optionsContainer: tailwind('bg-white rounded-lg shadow-xl') })}>
