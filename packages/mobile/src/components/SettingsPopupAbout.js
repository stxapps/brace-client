  ...
  return (
    <View style={tailwind('p-4 md:p-6')}>
      <View style={tailwind('border-b border-gray-200 md:hidden')}>
        ...
        <Text style={tailwind('pb-2 text-xl font-medium leading-6 text-gray-800')}>About</Text>
      </View>
      <View style={tailwind('mt-6 md:mt-0')}>
        ...
        <View style={tailwind('items-start justify-start pt-1 md:flex-row md:pt-10')}>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING)} style={tailwind('text-base font-normal text-gray-500')}>Home page</Text>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING_HOW)} style={tailwind('mt-4 text-base font-normal text-gray-500 md:mt-0 md:ml-14')}>How to</Text>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING_MOBILE)} style={tailwind('mt-4 text-base font-normal text-gray-500 md:mt-0 md:ml-14')}>Mobile apps</Text>
        </View>
      </View>
      <View style={tailwind('items-start justify-start pt-12 md:flex-row md:pt-10')}>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_ABOUT)} style={tailwind('text-base font-normal text-gray-500')}>About</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_TERMS)} style={tailwind('mt-4 text-base font-normal text-gray-500 md:mt-0 md:ml-14')}>Terms</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_PRIVACY)} style={tailwind('mt-4 text-base font-normal text-gray-500 md:mt-0 md:ml-14')}>Privacy</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_SUPPORT)} style={tailwind('mt-4 text-base font-normal text-gray-500 md:mt-0 md:ml-14')}>Support</Text>
      </View>
      <View style={tailwind('flex-row items-center justify-start pt-16 md:pt-16')}>
        <TouchableOpacity onPress={() => Linking.openURL('https://twitter.com/bracedotto')} style={tailwind('my-0.5')}>
          <Svg style={tailwind('font-normal text-gray-400')} width={34} height={27.2} viewBox="0 0 40 32" fill="currentColor">
        ...
        <TouchableOpacity onPress={() => Linking.openURL('https://github.com/stxapps/brace-client')} style={tailwind('ml-8')}>
          <Svg style={tailwind('font-normal text-gray-400')} width={32} height={31.02} viewBox="0 0 33 32" fill="currentColor">
      ...
      <View style={tailwind('items-start justify-start pt-16 lg:flex-row lg:justify-between')}>
        <Text style={tailwind('text-base font-normal text-gray-400')}>© 2022 <Text onPress={() => Linking.openURL('https://www.stxapps.com')} style={tailwind('text-base font-normal text-gray-400')}>STX Apps Co., Ltd.</Text></Text>
        <Text style={tailwind('text-base font-normal text-gray-400')}>Crafted with ❤ and ℅ for a better web</Text>
      </View>
    </View>
  );
};
