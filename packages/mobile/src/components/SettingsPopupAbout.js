...
import {
  ...
  HASH_TERMS, HASH_PRIVACY, HASH_SUPPORT,
} from '../types/const';
  ...
  return (
    <View style={tailwind('p-4 md:p-6', safeAreaWidth)}>
      <View style={tailwind('border-b border-gray-200 md:hidden', safeAreaWidth)}>
        ...
        <Text style={tailwind('pb-2 text-xl text-gray-800 font-medium leading-6')}>About</Text>
      </View>
      <View style={tailwind('mt-6 md:mt-0', safeAreaWidth)}>
        <View style={tailwind('hidden md:flex', safeAreaWidth)}>
          <SvgXml width={24.82} height={32} xml={shortLogo} />
        </View>
        <View style={tailwind('pt-1 justify-start items-start md:flex-row md:pt-10', safeAreaWidth)}>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING)} style={tailwind('text-base text-gray-500 font-normal')}>Home page</Text>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING_HOW)} style={tailwind('mt-4 text-base text-gray-500 font-normal md:mt-0 md:ml-14', safeAreaWidth)}>How to</Text>
          <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_LANDING_MOBILE)} style={tailwind('mt-4 text-base text-gray-500 font-normal md:mt-0 md:ml-14', safeAreaWidth)}>Mobile apps</Text>
        </View>
      </View>
      <View style={tailwind('pt-12 justify-start items-start md:flex-row md:pt-10', safeAreaWidth)}>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_ABOUT)} style={tailwind('text-base text-gray-500 font-normal')}>About</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_TERMS)} style={tailwind('mt-4 text-base text-gray-500 font-normal md:mt-0 md:ml-14', safeAreaWidth)}>Terms</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_PRIVACY)} style={tailwind('mt-4 text-base text-gray-500 font-normal md:mt-0 md:ml-14', safeAreaWidth)}>Privacy</Text>
        <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_SUPPORT)} style={tailwind('mt-4 text-base text-gray-500 font-normal md:mt-0 md:ml-14', safeAreaWidth)}>Support</Text>
      </View>
      <View style={tailwind('pt-16 flex-row justify-start items-center md:pt-16', safeAreaWidth)}>
        <TouchableOpacity onPress={() => Linking.openURL('https://twitter.com/bracedotto')} style={tailwind('my-0.5')}>
          <Svg style={tailwind('text-gray-400 font-normal')} width={34} height={27.2} viewBox="0 0 40 32" fill="currentColor">
        ...
        <TouchableOpacity onPress={() => Linking.openURL('https://github.com/stxapps/brace-client')} style={tailwind('ml-8')}>
          <Svg style={tailwind('text-gray-400 font-normal')} width={32} height={31.02} viewBox="0 0 33 32" fill="currentColor">
      ...
      <View style={tailwind('pt-16 justify-start items-start lg:flex-row lg:justify-between', safeAreaWidth)}>
        <Text style={tailwind('text-base text-gray-400 font-normal')}>© 2022 <Text onPress={() => Linking.openURL('https://www.stxapps.com')} style={tailwind('text-base text-gray-400 font-normal')}>STX Apps Co., Ltd.</Text></Text>
        <Text style={tailwind('text-base text-gray-400 font-normal')}>Crafted with ❤ and ℅ for a better web</Text>
      </View>
    </View>
  );
};
