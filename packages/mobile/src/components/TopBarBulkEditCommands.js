...
class TopBarBulkEditCommands extends React.Component {
  ...
  renderBulkEditMoveToPopup() {
    ...
    if (!styles.moveToMenuOption) styles.moveToMenuOption = { optionWrapper: { padding: 0 } };
    ...
    return (
      <React.Fragment>
        {moveTo.map(listNameObj => {
          ...
          return (
            <MenuOption key={key} onSelect={() => this.onBulkEditMoveToPopupClick(key)} customStyles={styles.moveToMenuOption}>
  ...
  render() {
    ...
    if (!styles.innerBtn) {
      ...
      styles.innerBtn = [tailwind('flex-row justify-center items-center bg-white border border-gray-700 rounded-full shadow-sm'), btnStyle];
    }

    if (!styles.moveToRendererProps) {
      const anchorClasses = Platform.select({ ios: 'z-10', android: 'shadow-xl' })
      styles.moveToRendererProps = { preferredPlacement: 'bottom', anchorStyle: tailwind(anchorClasses) };
    }

    if (!styles.moveToMenuOptions) styles.moveToMenuOptions = { optionsContainer: tailwind('py-2 bg-white rounded-lg shadow-xl') };
    ...
    return (
      <View style={tailwind('flex-row justify-end items-center')}>
        ...
        {isMoveToBtnShown && <Menu renderer={renderers.Popover} rendererProps={styles.moveToRendererProps} onOpen={this.onBulkEditMoveToBtnClick} onClose={this.onBulkEditMoveToCancelBtnClick}>
          <MenuTrigger>
          ...
          <MenuOptions customStyles={styles.moveToMenuOptions}>
