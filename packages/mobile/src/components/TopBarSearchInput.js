...
class TopBarSearchInput extends React.PureComponent {
  ...
  render() {
    ...
    const searchInputStyle = Platform.OS === 'ios' ? { height: 34 } : { paddingTop: 2, paddingBottom: 2 };

    return (
      <View style={tailwind('ml-4 w-48')}>
        <TextInput onChange={this.onSearchInputChange} style={cache('TBSI_searchInput', [tailwind('ml-0.5 pl-9 pr-6 w-full bg-gray-100 text-sm text-gray-700 font-normal border border-transparent rounded-full'), searchInputStyle])} placeholder="Search" placeholderTextColor="rgb(107, 114, 128)" value={searchString} autoCapitalize="none" />
        ...
        <View style={tailwind('pl-3 absolute inset-y-0 left-0 justify-center')}>
          <Svg style={tailwind('text-gray-400 font-normal')} width={20} height={20} viewBox="0 0 24 24" fill="currentColor">
