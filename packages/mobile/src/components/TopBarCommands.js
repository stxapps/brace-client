    ...
    return (
      <React.Fragment>
        ...
        <MenuOption onSelect={this.onSupportBtnClick} customStyles={cache('TBC_profileMenuOption', { optionWrapper: { padding: 0 } })}>
          <Text style={tailwind('py-2.5 pl-4 text-sm font-normal text-gray-700')}>Support</Text>
        </MenuOption>
        <MenuOption onSelect={this.onSignOutBtnClick} customStyles={cache('TBC_profileMenuOption', { optionWrapper: { padding: 0 } })}>
          <Text style={tailwind('py-2.5 pl-4 text-sm font-normal text-gray-700')}>Sign out</Text>
        </MenuOption>
      </React.Fragment>
    );
  }
    ...
    return (
      <View style={tailwind('flex-row items-center justify-end')}>
        ...
        <TouchableOpacity onPress={this.onBulkEditBtnClick} style={tailwind('ml-4')}>
          <View style={cache('TBC_bulkEditBtnView', [tailwind('flex-row items-center justify-center rounded-full border border-gray-400 bg-white px-3'), { height: 32, paddingLeft: 10, paddingRight: 12 }])}>
            <Svg style={tailwind('font-normal text-gray-500')} width={14} height={14} viewBox="0 0 20 20" fill="currentColor">
            ...
            <Text style={tailwind('ml-1 text-sm font-normal text-gray-500')}>Select</Text>
          </View>
        </TouchableOpacity>
        <Menu renderer={MenuPopoverRenderers} rendererProps={cache('TBC_profileCommandMenuRendererProps', { preferredPlacement: 'bottom', anchorStyle: tailwind(anchorClasses) })} onOpen={this.onProfileBtnClick} onClose={this.onProfileCancelBtnClick}>
          <MenuTrigger>
            <View style={tailwind('ml-4')}>
              <View style={tailwind('h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-white')}>
                <Svg style={tailwind('font-normal text-gray-500')} width={28} height={28} viewBox="0 0 24 24" stroke="currentColor" fill="none">
          ...
          <MenuOptions customStyles={cache('TBC_profileCommandMenuOptions', { optionsContainer: tailwind('w-28 rounded-lg bg-white py-2 shadow-xl') })}>
