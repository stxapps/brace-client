  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <div className={tailwind(`fixed inset-x-0 bottom-0 z-41 transform rounded-t-lg bg-white px-4 pt-6 pb-6 shadow-xl ring-1 ring-black ring-opacity-5 ${!isAddPopupShown ? 'translate-y-full' : ''}`)}>
          <div className={tailwind('flex')}>
            ...
            <div className={tailwind('ml-3 flex-1')}>
              <input ref={this.addInput} onChange={this.onAddInputChange} onKeyPress={this.onAddInputKeyPress} className={tailwind('w-full rounded-full border border-gray-400 bg-white px-3.5 py-1 text-base text-gray-700 focus:border-gray-400 focus:outline-none focus:ring focus:ring-blue-500 focus:ring-opacity-50')} type="url" placeholder="https://" value={url} autoCapitalize="none" />
            </div>
          </div>
