        ...
        <div className={tailwind('min-w-0 flex-1 pb-3.5 pl-3 sm:pl-4')}>
          <div className={tailwind('flex flex-wrap items-center justify-start pt-1')}>
            {tnAndDns.map((tnAndDn, i) => {
              return (
                <button key={tnAndDn.tagName} onClick={() => dispatch(updateQueryString(tnAndDn.tagName))} className={tailwind(`group mt-2 block rounded-full bg-gray-100 px-2 py-1 hover:bg-gray-200 focus:outline-none focus:ring blk:bg-gray-700 blk:hover:bg-gray-600 ${i === 0 ? '' : 'ml-2'}`)}>
