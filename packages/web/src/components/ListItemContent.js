      ...
      <div className={tailwind('flex items-center')}>
        ...
        <div className={tailwind('min-w-0 flex-1 py-3.5 pl-3 sm:pl-4')}>
          {tnAndDns.map(tnAndDn => {
            return (
              <button key={tnAndDn.tagName} onClick={() => updateQueryString(tnAndDn.tagName)} className={tailwind('block text-sm text-gray-600 font-medium border border-gray-200 rounded-full')}>{tnAndDn.displayName}</button>
            );
          })}
        </div>
      </div>
