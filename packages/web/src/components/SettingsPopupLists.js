    ...
    if (listNameObj && listNameObj.children && listNameObj.children.length > 0) {
      ...
      expandBtn = (
        <button onClick={onExpandBtnClick} className={tailwind('group flex h-10 w-8 flex-shrink-0 flex-grow-0 items-center justify-start focus:outline-none')}>
          <div className={tailwind('flex h-3.5 w-3.5 items-center justify-center rounded-sm text-gray-500 group-hover:text-gray-600 group-focus:ring group-focus:ring-offset-4 blk:text-gray-400 blk:group-hover:text-gray-200 blk:group-focus:ring-offset-gray-900')}>
            {expandSvg}
          </div>
        </button>
      );
