...
class _ListNameEditor extends React.PureComponent {
  ...
  onInputBlur = () => {
    ...
    // If blur because of clicking ok, should not cancel

    // If blur on existing and there is a change? should cancel?

    // If click ok, and there is an error, should refocus?

    // If one list is being edited, other list can be edit?
  ...
  render() {
    ...
    const { listNameObj } = this.props;
    const { mode, value, msg } = this.state;

    // No delete on 3 main lists, make visible hidden
    ...
    return (
      <div className="mt-2 flex justify-start items-center">
        {(mode === MODE_VIEW && listNameObj === null) && <button onClick={this.onAddBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 focus:outline-none">
          <svg style={{ width: '0.875rem', height: '0.875rem' }} className="text-gray-700" viewBox="0 0 14 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        {(mode === MODE_VIEW && listNameObj !== null) && <button onClick={this.onDeleteBtnClick} className={`flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 ${deleteBtnStyleClasses} focus:outline-none`}>
          <svg className="h-4 text-gray-600" viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        {mode === MODE_EDIT && <button onClick={this.onCancelBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 focus:outline-none">
          <svg className="w-3 h-3 text-gray-700" viewBox="0 0 12 12" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="relative flex-grow flex-shrink">
          <input ref={this.input} onFocus={this.onInputFocus} onBlur={this.onInputBlur} onChange={this.onInputChange} onKeyPress={this.onInputKeyPress} className="py-2 w-full bg-white text-base text-gray-900 border-0 appearance-none focus:outline-none" type="text" placeholder="Create new list" value={value} />
          <p style={{ bottom: '-0.5rem' }} className="absolute left-0 text-sm text-red-600 font-medium leading-5">{msg}</p>
        </div>
        {mode === MODE_EDIT && <button onTouchStart={this.onOkBtnPress} onMouseDown={this.onOkBtnPress} onClick={this.onOkBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 focus:outline-none">
          <svg className="w-4 text-gray-700" viewBox="0 0 14 10" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        {(mode === MODE_VIEW && listNameObj !== null) && <button onClick={this.onEditBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 focus:outline-none">
          <svg className="w-4 h-4 text-gray-700" viewBox="0 0 14 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        {(mode === MODE_VIEW && listNameObj !== null) && <button onClick={this.onMoveUpBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 focus:outline-none">
          <svg className="h-4 text-gray-700" viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        {(mode === MODE_VIEW && listNameObj !== null) && <button onClick={this.onMoveDownBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 focus:outline-none">
          <svg className="h-4 text-gray-700" viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
