...
import {
  HASH_SUPPORT, SIGN_UP_POPUP, SIGN_IN_POPUP, MY_LIST,
...
class Adding extends React.PureComponent {
  ...
  _render(content) {
    ...
    return (
      <React.Fragment>
        ...
        <div className={tailwind('mx-auto w-full max-w-md bg-white px-4 pt-28 md:px-6 md:pt-36 lg:px-8')}>
          {content}
        </div>
        <SignUpPopup />
        <SignInPopup />
  ...
  renderNav() {
    ...
    let rightLink = <a className={tailwind('block rounded-sm text-right text-base font-medium leading-none text-gray-500 hover:text-gray-600 focus:outline-none focus:ring')} href="/">{isUserSignedIn ? 'Go to My List >' : 'Go to Brace.to >'}</a>
    let centerText = null;
    let leftLink = urlValidatedResult === VALID_URL ? <a className={tailwind('mt-6 block rounded-sm text-left text-base leading-none text-gray-500 hover:text-gray-600 focus:outline-none focus:ring md:mt-0')} href={addingPUrl}>Back to the link</a> : <div />;

    if (param && param[URL_QUERY_CLOSE_KEY]) {
      if (param[URL_QUERY_CLOSE_KEY] === URL_QUERY_CLOSE_WINDOW) {
        leftLink = null;
        centerText = <button onClick={() => window.close()} className={tailwind('block w-full rounded-sm py-2 text-center text-base text-gray-500 hover:text-gray-600 focus:outline-none focus:ring')}>Close this window</button>;
        rightLink = null;
      }
    }
  ...
  renderAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-gray-500')}>
          <a className={tailwind('break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring')} href={addingPUrl} target="_blank" rel="noreferrer">{addingTUrl}</a>
          <br />
          <span className={tailwind('break-normal text-lg font-semibold text-gray-900')}>is being saved.</span>
        </p>
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-gray-500')}>
          <a className={tailwind('break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring')} href={addingPUrl} target="_blank" rel="noreferrer">{addingTUrl}</a>
          <br />
          <span className={tailwind('break-normal text-lg font-semibold text-gray-900')}>has been saved.</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderDiedAdding() {
    ...
    const content = (
      <React.Fragment>
        <svg className={tailwind('mx-auto h-24 text-red-600')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-lg font-semibold text-gray-800')}>Oops..., something went wrong!</p>
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-left text-base leading-relaxed text-gray-500 sm:text-center')}>Please wait a moment and try again. If the problem persists, please <a className={tailwind('rounded underline hover:text-gray-600 focus:outline-none focus:ring')} href={'/' + HASH_SUPPORT} target="_blank" rel="noreferrer">contact us
        ...
        <button onClick={() => this.addLink(true)} className={tailwind('group mx-auto mt-5 mb-px block h-14 focus:outline-none')}>
          <span className={tailwind('rounded-full border border-gray-400 bg-white px-3 py-2 text-sm text-gray-500 group-hover:border-gray-500 group-hover:text-gray-600 group-focus:ring')}>Try again</span>
        </button>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderAskingConfirm() {
    ...
    const content = (
      <React.Fragment>
        <svg className={tailwind('mx-auto h-24 text-yellow-500')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-gray-500')}>
          <a className={tailwind('break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring')} href={addingPUrl} target="_blank" rel="noreferrer">{addingTUrl}</a>
          <br />
          <span className={tailwind('break-normal text-lg font-semibold text-gray-900')}>looks like an invalid link. Are you sure?</span>
        </p>
        <button onClick={this.onAskingConfirmOkBtnClick} className={tailwind('group mx-auto mt-5 mb-px block h-14 focus:outline-none')}>
          <span className={tailwind('rounded-full border border-gray-400 bg-white px-3 py-2 text-sm text-gray-500 group-hover:border-gray-500 group-hover:text-gray-600 group-focus:ring')}>Yes, I'm sure</span>
        </button>
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-sm tracking-wide text-gray-500')}>You can edit the link in address bar and press enter again</p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderInvalid() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-red-700')}>{msg}</p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderNotSignedIn() {
    ...
    const content = (
      <React.Fragment>
        <svg className={tailwind('mx-auto h-24 text-yellow-600')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" clipRule="evenodd" d="M8.25706 3.09882C9.02167 1.73952 10.9788 1.73952 11.7434 3.09882L17.3237 13.0194C18.0736 14.3526 17.1102 15.9999 15.5805 15.9999H4.4199C2.89025 15.9999 1.92682 14.3526 2.67675 13.0194L8.25706 3.09882ZM11.0001 13C11.0001 13.5523 10.5524 14 10.0001 14C9.44784 14 9.00012 13.5523 9.00012 13C9.00012 12.4477 9.44784 12 10.0001 12C10.5524 12 11.0001 12.4477 11.0001 13ZM10.0001 5C9.44784 5 9.00012 5.44772 9.00012 6V9C9.00012 9.55228 9.44784 10 10.0001 10C10.5524 10 11.0001 9.55228 11.0001 9V6C11.0001 5.44772 10.5524 5 10.0001 5Z" />
        </svg>
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-gray-500')}>Please sign in first</p>
        <button onClick={this.onSignInBtnClick} className={tailwind('group mx-auto mt-2 block h-14 focus:outline-none')}>
          <span className={tailwind('rounded-full border border-gray-400 bg-white px-3 py-1.5 text-sm text-gray-500 group-hover:border-gray-500 group-hover:text-gray-600 group-focus:ring')}>Sign in</span>
        </button>
        <div className={tailwind('mt-10 flex items-center justify-center')}>
          <p className={tailwind('text-base text-gray-500')}>No account yet?</p>
          <button onClick={this.onSignUpBtnClick} className={tailwind('ml-2 rounded-sm text-base font-medium text-gray-600 hover:text-gray-700 focus:outline-none focus:ring')}>Sign up</button>
        </div>
        {this.renderNav()}
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <p className={tailwind('mx-auto mt-5 w-full max-w-xs text-center text-base text-gray-500')}>
          <a className={tailwind('break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring')} href={addingPUrl} target="_blank" rel="noreferrer">{addingTUrl}</a>
          <br />
          <span className={tailwind('break-normal text-lg font-semibold text-gray-900')}>already exists</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
