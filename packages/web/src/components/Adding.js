...
class Adding extends React.Component {
  ...
  addLink(doCancelDiedLink = false) {
    ...
    } else if (href.startsWith(LOCAL_IP)) {
      addingUrl = href.substring(LOCAL_IP.length);
    }
  ...
  renderNav() {
    ...
    let rightLink = <a className="block text-2xl text-gray-900 font-semibold text-right leading-none hover:underline" href="/">{isUserSignedIn ? 'Go to My List >' : 'Go to Brace.to >'}</a>
    let centerText = null;
    let leftLink = isValid ? <a className="mt-6 block text-base text-gray-900 text-left leading-none hover:underline md:mt-0" href={addingPUrl}>Back to the link</a> : <div></div>;
  ...
  renderAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-xl text-gray-900 text-center">
          <a className="break-all" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="font-semibold break-normal">is being saved.</span>
        </p>
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-xl text-gray-900 text-center">
          <a className="break-all" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="font-semibold break-normal">has been saved.</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderDiedAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-xl text-gray-900 text-center">Oops..., something went wrong! </p>
        ...
        <button onClick={() => this.addLink(true)} className="mx-auto mt-5 mb-px block h-14 ">
          <span className="px-4 py-2 text-base text-gray-900 border border-gray-900 rounded-full shadow-sm hover:bg-gray-800 hover:text-white active:bg-gray-900">Try again</span>
        </button>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderAskingConfirm() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-lg text-gray-900 text-center">
          <a className="break-all" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="font-semibold break-normal">looks like an invalid link. Are you sure?</span>
        </p>
        <button onClick={this.onAskingConfirmOkBtnClick} className="mx-auto mt-5 mb-px block h-14 ">
          <span className="px-4 py-2 text-base text-gray-900 border border-gray-900 rounded-full shadow-sm hover:bg-gray-800 hover:text-white active:bg-gray-900">Yes, I'm sure</span>
        </button>
  ...
  renderInvalid() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-lg text-gray-900 text-center">{msg}</p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderNotSignedIn() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-base text-gray-900 text-center">Please sign in first</p>
        <button onClick={() => this.props.signIn()} className="mx-auto mb-px block h-14 ">
          <span className="px-4 py-2 text-base text-gray-900 border border-gray-900 rounded-full shadow-sm hover:bg-gray-800 hover:text-white active:bg-gray-900">Sign in</span>
        </button>
        <div className="mt-10 flex justify-center items-center">
          <p className="text-base text-gray-900">No account yet?</p>
          <button onClick={() => this.props.signUp()} className="ml-2 hover:underline">Sign up</button>
        </div>
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-xl text-gray-900 text-center">
          <a className="break-all" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="font-semibold break-normal">already exists.</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
