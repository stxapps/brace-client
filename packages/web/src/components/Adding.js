...
class Adding extends React.PureComponent {
  ...
  renderNav() {
    ...
    let rightLink = <a className="block text-base text-gray-500 font-medium text-right leading-none rounded-sm hover:text-gray-600 focus:outline-none focus:ring" href="/">{isUserSignedIn ? 'Go to My List >' : 'Go to Brace.to >'}</a>
    let centerText = null;
    let leftLink = urlValidatedResult === VALID_URL ? <a className="mt-6 block text-sm text-gray-500 text-left leading-none tracking-wide rounded-sm hover:text-gray-600 focus:outline-none focus:ring md:mt-0" href={addingPUrl}>Back to the link</a> : <div />;

    if (param && param[URL_QUERY_CLOSE_KEY]) {
      if (param[URL_QUERY_CLOSE_KEY] === URL_QUERY_CLOSE_WINDOW) {
        leftLink = null;
        centerText = <button onClick={() => window.close()} className="py-2 block w-full text-sm text-gray-500 text-center tracking-wide rounded-sm hover:text-gray-600 focus:outline-none focus:ring">Close this window</button>;
        rightLink = null;
      }
    }
  ...
  renderAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center">
          <a className="break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="text-base text-gray-900 font-medium break-normal">is being saved.</span>
        </p>
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center">
          <a className="break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="text-base text-gray-900 font-medium break-normal">has been saved.</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderDiedAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-base text-gray-800 font-medium text-center">Oops..., something went wrong!</p>
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center tracking-wide leading-relaxed">Please wait a moment and try again. If the problem persists, please <a className="underline rounded hover:text-gray-600 focus:outline-none focus:ring" href="/#support">contact us
        ...
        <button onClick={() => this.addLink(true)} className="mx-auto mt-5 mb-px block h-14 group focus:outline-none">
          <span className="px-3 py-1.5 bg-white text-sm text-gray-500 border border-gray-400 rounded-full shadow-sm group-hover:text-gray-600 group-hover:border-gray-500 group-focus:ring">Try again</span>
        </button>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderAskingConfirm() {
    ...
    const content = (
      <React.Fragment>
        <svg className="mx-auto h-24 text-yellow-500" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center">
          <a className="break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="text-base text-gray-900 font-medium break-normal">looks like an invalid link. Are you sure?</span>
        </p>
        <button onClick={this.onAskingConfirmOkBtnClick} className="mx-auto mt-5 mb-px block h-14 group focus:outline-none">
          <span className="px-3 py-1.5 bg-white text-sm text-gray-500 border border-gray-400 rounded-full shadow-sm group-hover:text-gray-600 group-hover:border-gray-500 group-focus:ring">Yes, I'm sure</span>
        </button>
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center tracking-wide">You can edit the link in address bar and press enter again</p>
        {this.renderNav()}
      </React.Fragment>
    );
  ...
  renderInvalid() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-base text-gray-500 text-center">{msg}</p>
        {this.renderNav()}
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <button onClick={() => this.props.signIn()} className="mx-auto mt-2 block h-14 group focus:outline-none">
          <span className="px-3 py-1.5 bg-white text-sm text-gray-500 border border-gray-400 rounded-full shadow-sm group-hover:text-gray-600 group-hover:border-gray-500 group-focus:ring">Sign in</span>
        </button>
        <div className="mt-10 flex justify-center items-center">
          <p className="text-sm text-gray-500">No account yet?</p>
          <button onClick={() => this.props.signUp()} className="ml-2 text-sm text-gray-600 font-medium rounded-sm hover:text-gray-700 focus:outline-none focus:ring">Sign up</button>
        </div>
        {this.renderNav()}
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-sm text-gray-500 text-center">
          <a className="break-all rounded-sm hover:text-gray-600 focus:outline-none focus:ring" href={addingPUrl}>{addingTUrl}</a>
          <br />
          <span className="text-base text-gray-900 font-medium break-normal">already exists.</span>
        </p>
        {this.renderNav()}
      </React.Fragment>
    );
