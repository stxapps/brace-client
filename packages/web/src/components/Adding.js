...
class Adding extends React.PureComponent {
  ...
  renderNav() {
    ...
    let centerText = null;
    let leftLink = urlValidatedResult === VALID_URL ? <a className="mt-6 block text-base text-gray-500 text-left leading-none rounded-sm hover:text-gray-600 focus:outline-none focus:ring md:mt-0" href={addingPUrl}>Back to the link</a> : <div />;

    if (param && param[URL_QUERY_CLOSE_KEY]) {
      if (param[URL_QUERY_CLOSE_KEY] === URL_QUERY_CLOSE_WINDOW) {
        leftLink = null;
        centerText = <button onClick={() => window.close()} className="py-2 block w-full text-base text-gray-500 text-center rounded-sm hover:text-gray-600 focus:outline-none focus:ring">Close this window</button>;
        rightLink = null;
      }
    }
  ...
  renderDiedAdding() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-lg text-gray-800 font-semibold text-center">Oops..., something went wrong!</p>
        <p className="mx-auto mt-5 w-full max-w-xs text-base text-gray-500 text-left leading-relaxed sm:text-center">Please wait a moment and try again. If the problem persists, please <a className="underline rounded hover:text-gray-600 focus:outline-none focus:ring" href={'/' + HASH_SUPPORT} target="_blank" rel="noreferrer">contact us
        ...
        <button onClick={() => this.addLink(true)} className="mx-auto mt-5 mb-px block h-14 group focus:outline-none">
          <span className="px-3 py-2 bg-white text-sm text-gray-500 border border-gray-400 rounded-full group-hover:text-gray-600 group-hover:border-gray-500 group-focus:ring">Try again</span>
        </button>
        {this.renderNav()}
      </React.Fragment>
    );
    ...
    const content = (
      <React.Fragment>
        ...
        <button onClick={this.onAskingConfirmOkBtnClick} className="mx-auto mt-5 mb-px block h-14 group focus:outline-none">
          <span className="px-3 py-2 bg-white text-sm text-gray-500 border border-gray-400 rounded-full group-hover:text-gray-600 group-hover:border-gray-500 group-focus:ring">Yes, I'm sure</span>
        </button>
  ...
  renderInvalid() {
    ...
    const content = (
      <React.Fragment>
        ...
        <p className="mx-auto mt-5 w-full max-w-xs text-base text-red-700 text-center">{msg}</p>
        {this.renderNav()}
      </React.Fragment>
    );
