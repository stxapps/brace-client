...
const ListItemSelector = (props) => {
  ...
  return (
    <button onClick={onSelectBtnClick} className="absolute inset-0 flex items-center w-full h-full bg-transparent group focus:outline-none">
      <div className="flex-grow-0 flex-shrink-0 w-16 h-full bg-white pl-px flex justify-center items-center">
      ...
      <div className="flex-grow-0 flex-shrink-0 w-3 h-full bg-transparent -mr-3 sm:w-1 sm:-mr-1" />
    </button>
  );
};
