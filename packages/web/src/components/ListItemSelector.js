...
const ListItemSelector = (props) => {
  ...
  return (
    <button onClick={onSelectBtnClick} className={tailwind('group absolute inset-0 flex h-full w-full items-center bg-transparent focus:outline-none')}>
      <div className={tailwind('flex h-full w-16 flex-shrink-0 flex-grow-0 items-center justify-center bg-white pl-px')}>
        <div className={tailwind(`flex h-10 w-10 items-center justify-center rounded-full border group-hover:ring group-focus:ring ${circleClassNames}`)}>
