...
class BottomBar extends React.Component {
  ...
  renderAddPopup() {
    ...
    return (
      <React.Fragment>
        <button onClick={this.onAddCancelBtnClick} tabIndex={-1} className={`${!isAddPopupShown ? 'hidden' : ''} fixed inset-0 w-full h-full bg-black opacity-50 cursor-default z-40 focus:outline-none`}></button>
        <div className={`px-4 pt-6 pb-6 fixed inset-x-0 bottom-0 bg-white border border-gray-200 rounded-t-lg shadow-xl transform ${!isAddPopupShown ? 'translate-y-full' : ''} transition duration-300 ease-in-out z-41`}>
          <input ref={this.addInput} onChange={this.onAddInputChange} onKeyPress={this.onAddInputKeyPress} className="px-4 py-2 w-full bg-white text-gray-900 border border-gray-600 rounded-full focus:outline-none focus:shadow-outline" type="text" placeholder="https://" value={url} />
    ...
    return (
      <div style={style} className={`px-2 py-2 fixed inset-x-0 flex justify-between items-center bg-white border border-gray-200 transform ${!isSearchPopupShown ? 'translate-y-full' : ''} transition duration-200 ease-in-out z-10`}>
        <div className="relative w-full">
          <input ref={this.searchInput} onChange={this.onSearchInputChange} className="pl-4 pr-8 py-1 flex-grow-1 flex-shrink w-full bg-white text-gray-900 border border-gray-600 rounded-full focus:outline-none focus:shadow-outline" type="text" placeholder="Search" value={searchString} />
  ...
  renderProfilePopup() {
    ...
    return (
      <React.Fragment>
        <button onClick={this.onProfileCancelBtnClick} tabIndex={-1} className={`${!isProfilePopupShown ? 'hidden' : ''} fixed inset-0 w-full h-full bg-black opacity-50 cursor-default z-40 focus:outline-none`}></button>
