  ...
  renderProfilePopup() {
    ...
    return (
      <AnimatePresence key="AnimatePresence_ProfilePopup">
        ...
        <motion.div key="ProfilePopup_menuPopup" className={tailwind('absolute right-0 z-41 mt-2 w-28 origin-top-right rounded-lg bg-white py-2 shadow-xl ring-1 ring-black ring-opacity-5')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden">
          <button onClick={this.onSettingsBtnClick} className={tailwind('block w-full rounded-md py-2.5 pl-4 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring focus:ring-inset')}>Settings</button>
          <button onClick={this.onSupportBtnClick} className={tailwind('block w-full rounded-md py-2.5 pl-4 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring focus:ring-inset')}>Support</button>
          <button onClick={this.onSignOutBtnClick} className={tailwind('block w-full rounded-md py-2.5 pl-4 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring focus:ring-inset')}>Sign out</button>
        </motion.div>
      </AnimatePresence>
    );
  }
    ...
    return (
      <div className={tailwind('flex items-center justify-end')}>
        ...
        <div className={tailwind('relative ml-4')}>
          <button onClick={this.onBulkEditBtnClick} style={{ height: '2rem', paddingLeft: '0.625rem', paddingRight: '0.75rem' }} className={tailwind('group flex items-center justify-center rounded-full border border-gray-400 bg-white px-3 hover:border-gray-500 focus:outline-none focus:ring')}>
            <svg className={tailwind('mx-auto w-3.5 text-gray-500 group-hover:text-gray-600')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className={tailwind('ml-1 text-sm text-gray-500 group-hover:text-gray-600')}>Select</span>
          </button>
        </div>
        <div className={tailwind('relative ml-4')}>
          <button onClick={this.onProfileBtnClick} className={tailwind(`group relative block h-8 w-8 overflow-hidden rounded-full focus:outline-none focus:ring ${isProfilePopupShown ? 'z-41' : ''}`)}>
            <svg className={tailwind('mx-auto w-7 text-gray-500 group-hover:text-gray-600')} viewBox="0 0 24 24" stroke="currentColor" fill="none" xmlns="http://www.w3.org/2000/svg">
