...
- As startup script is executed by root user, need to specify running commands by wit user
- use 'sudo -i' to switch to root user and test the script below
- startup script in Custom metadata, key: startup-script, value:
  #! /bin/bash
  sudo -u wit bash -c 'cd /home/wit/brace-server-worker; mkdir -p ./logs; NOW=$(date +'%Y%m%d%H%M%S'); /home/wit/.nvm/versions/node/v12.22.1/bin/node index.js > "./logs/log_${NOW}.txt" 2>&1; sudo shutdown -h now'
