...
- npm i file:../../../sdrive-hub-sdk/blockstack-react-native/react-native-blockstack-0.1.1.tgz
  + In the blockstack-react-native, run npm pack
...
- ios
  ...
  + targets: Brace, on General:
    ~ update Version, App Category: Productivity, Display Name: Brace
  + targets: Brace, on Signing & Capabilities, add In-App Purchase
  + targets: Brace, on Build Phases, add Target Dependency: SavetoBrace
  + targets: SavetoBrace, on General:
    ~ update Version, Miminum iOS, Display Name: Save to Brace
...
** Steps on AWS
1.1 In AWS S3 -> Permissions -> CORS
    [
        {
            "AllowedHeaders": [
                "*"
...
1.2 Create custom cache policy: CustomUseOriginCacheControlHeaders-QueryStrings
1.3 Create custom origin request policy: CustomCORS-S3Origin
1.4 Create CF functions: test-brace-web-rewrite, brace-web-rewrite
2.1 upload files to S3, don't invalidate CF yet!
2.2 In AWS S3 -> Properties -> Static Website Hosting -> Error document change to 404.html
2.3 Set CF behavior -> Cache key and origin requests to recommended
    Cache: CustomUseOriginCacheControlHeaders-QueryStrings
    Origin request: Managed-CORS-S3Origin
2.4 Set CF behavior -> Function associations -> Viewer request -> brace-web-rewrite
2.5 invalidate CF
